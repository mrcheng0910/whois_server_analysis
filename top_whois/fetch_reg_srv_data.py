...
def registrar_srvs(end_tb):
    ...
    for table_num in xrange(1, end_tb):
        ...
        sql = """SELECT sponsoring_registrar, sec_whois_server FROM domain_whois_com_top_srv_{n} WHERE sec_whois_server!='' AND sec_whois_server IS NOT NULL AND sec_whois_server!='0' GROUP BY sponsoring_registrar, sec_whois_server""".format(n=table_num)
