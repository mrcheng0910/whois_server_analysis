...
class Database:
    ...
    def __init__(self, host='172.26.253.3', user='root', passwd='platform', db='cyn_test'):
        """
        数据库初始化
        """
        self.host = host
...
# sql = 'select creation_date,domain from whois WHERE creation_date != "" '
...
for i in domains:
    ...
    try:
        y = datetime.datetime.strptime(ud, '%Y-%m-%d')
        during_day = (c-y).days
        if during_day <= 82:
            # print during_day
            print i[1]
...
# print (c-datetime.datetime.strptime('2017-2-1','%Y-%m-%d')).days